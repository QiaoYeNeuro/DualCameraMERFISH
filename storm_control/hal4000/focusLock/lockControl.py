...
class LockControl(QtCore.QObject):
    ...
    def handleCheckFocusLock(self):
        ...
        if self.isGoodLock():
            ...
            self.lock_mode.scanedAndSuccess = False
        else:
            self.current_state["num_checks"] -= 1
            #Bogdan add 02/11/2025 for large chamber
            self.lock_mode.scanedAndSuccess = True 
    ...
    def handleDone(self, success):
        ...
        if self.current_state is not None:
            ...
            if tcp_message.isType("Check Focus Lock"):
                ...
                scanedAndSuccess = getattr(self.lock_mode,"scanedAndSuccess",True)
                tcp_message.addResponse("scanedAndSuccess", scanedAndSuccess)
    ...
    def stopFilm(self):
        if self.working:
            if self.offset_fp is not None:
                ...
                frames = [ln for ln in open(self.offset_file,'r')]
                if len(frames)>5:
                    good_offset = np.mean(np.array([frames[i][:-1].split(' ')[-1] for i in [1,2,3,4,5]])=='1')>0.5
                else:
                    good_offset=True
