...
class PhotometricsCameraControl(cameraControl.HWCameraControl):
    ...
    def __init__(self, config = None, is_master = False, **kwds):
        ...
        self.parameters.add(params.ParameterRangeInt(description = "Trigger Mode",
                                                     name = "triger_mode",
                                                     value = 0,
                                                     min_value = 0,
                                                     max_value = 1))
        self.parameters.add(params.ParameterRangeInt(description = "Expected Frames",
                                                     name = "exp_len",
                                                     value = 200,
                                                     min_value = 1,
                                                     max_value = 5000))
    ...
    def newParameters(self, parameters, initialization = False):
        ...
        if (len(to_change) > 0):
            ...
            # one indexed.
            #self.paramaters.
            #self.parameters.get("triger_mode")
            self.camera.triger_mode = parameters.get("triger_mode")
            self.camera.exp_len = parameters.get("exp_len")
