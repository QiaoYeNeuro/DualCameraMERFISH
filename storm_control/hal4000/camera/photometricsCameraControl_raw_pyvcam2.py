...
from storm_control.sc_hardware.photometrics.pyvcam import pvc
import storm_control.sc_hardware.photometrics.pyvcam.camera as Camera
from storm_control.sc_hardware.photometrics.pyvcam import constants as const
...
class PhotometricsCameraControl(cameraControl.HWCameraControl):
    ...
    def __init__(self, config = None, is_master = False, **kwds):
        ...
        name = Camera.get_available_camera_names()
        ...
        self.camera = Camera.select_camera(name)
        ...
        self.camera_functionality = cameraFunctionality.CameraFunctionality(camera_name = self.name,
        ...
        self.parameters.add(params.ParameterRangeInt(description = "Sensor readout speed",
                                                     ...
                                                     # Human-friendly trigger selector for HAL/GUI
              ##Qiao 08262025                                       
        self.parameters.add(params.ParameterSetString(
            name="param_trigger_mode",
            description="PVCAM trigger mode",
            value="ext_edge",             # default to external edge trigger; change to "internal" if you prefer
            allowed=["internal", "ext_edge", "ext_bulb"]
))
 ##---------------------
    ...
    def newParameters(self, parameters, initialization = False):
        ...
        if (len(to_change) > 0):
           ...
           # for pname in to_change:
            ...
            # Reconfigure the camera.
            # Map friendly strings -> PyVCAM constants where needed.
            trigger_map = {
                "internal": const.INTERNAL_TRIGGER,
                "ext_edge": const.EXT_TRIG_EDGE_RISING,
                "ext_bulb": const.EXT_TRIG_TRIG_FIRST,   # use the right constant for bulb/level if needed
            }


            for pname in to_change:
                if pname == "param_trigger_mode":
                    sel = parameters.get("param_trigger_mode")
                    if sel not in trigger_map:
                        raise halExceptions.HardwareException(
                            f"Unknown trigger mode '{sel}'. Allowed: {list(trigger_map.keys())}"
                        )
                    self.camera.exp_mode = trigger_map[sel]


                elif pname.startswith("param_"):
                    # All other PVCAM parameters go through the generic setter
                    self.camera.setParameter(pname, parameters.get(pname))
