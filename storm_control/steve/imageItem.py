...
class ImageItemLoaderHAL(object):
    ...
    def loadMovie(self, no_ext_name, frame_number):
        ...
        #print(no_ext_name, getCameraExtension(xml), xml.get("film.filetype"))
        # Load movie numpy data.
        extension = getCameraExtension(xml)
        if len(getCameraExtension(xml))>0:
            extension = '_c1'    
        mv_reader = movieReader.inferReader(no_ext_name + extension + xml.get("film.filetype"))
